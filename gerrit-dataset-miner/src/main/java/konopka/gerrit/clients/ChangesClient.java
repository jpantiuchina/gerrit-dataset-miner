...
public class ChangesClient
{
    ...
    public ChangesClient(GerritApi api, WaitCaller caller, IChangesRepository repository, ProjectsClient projects,
    ...
    private static PatchSetFileDto mapPatchSetFileDto(PatchSetDto patch, String path, FileInfo info)
    {
    ...
    public List<ChangeInfo> getChangeDetails(int number)
    {
        ...
        try
        {
            ...
            Changes.QueryRequest request = api.changes().query(query).withOptions(ListChangesOption.DETAILED_LABELS,
                    ListChangesOption.ALL_REVISIONS, ListChangesOption.ALL_COMMITS, ListChangesOption.ALL_FILES,
                    ListChangesOption.MESSAGES, ListChangesOption.CURRENT_ACTIONS, ListChangesOption
                            .DETAILED_ACCOUNTS, ListChangesOption.DOWNLOAD_COMMANDS);
    ...
    public List<ChangeInfo> downloadChanges(String query, int start, int limit)
    {
        ...
        try
        {
            ...
            Changes.QueryRequest request = api.changes().query(query).withStart(start).withLimit(limit);

            infos = caller.waitOrCall(request::get);
        }
        catch (Exception e)
        {
            logger.error(Logging.prepare("downloadChanges", query, Integer.toString(start), Integer.toString(limit)),
                    e);
        }
        if (infos == null || infos.size() == 0)
        {
            infos = Collections.emptyList();
            logger.info(Logging.prepareWithPart("downloadChanges", "no results", Integer.toString(start), Integer
                    .toString(limit)));
        }
        else
        {
            ...
            if (results.size() > 0)
            {
                logger.info(Logging.prepareWithPart("downloadChanges", String.join(",", results), Integer.toString
                        (start), Integer.toString(limit)));
            }
            else
            {
                logger.info(Logging.prepareWithPart("downloadChanges", "no results", Integer.toString(start), Integer
    ...
    public ChangeDto saveChange(ChangeInfo info) throws RestApiException, InterruptedException, SQLException
    {
        ...
        for (Map.Entry<String, RevisionInfo> entry : info.revisions.entrySet())
        {
            ...
            {
                patch.addedLines = revision.files.values().stream().filter(f -> f.linesInserted != null).mapToInt(f
                        -> f.linesInserted).sum();
                patch.deletedLines = revision.files.values().stream().filter(f -> f.linesDeleted != null).mapToInt(f
                        -> f.linesDeleted).sum();
            }
            ...
            {
                logger.info(Logging.prepareWithPart("saveChange", "comments: no comments", Integer.toString(info
                        ._number)), exception);
        ...
        Optional<PatchSetDto> current = change.patchSets.stream().filter(p -> p.gitCommitId.equals(info
                .currentRevision)).findFirst();
        ...
        if (info.labels != null && !info.labels.isEmpty())
        {
            ...
            info.labels.entrySet().stream().filter(e -> e.getValue() != null && e.getValue().all != null && e
                    .getValue().all.size() > 0).forEach(e -> e.getValue().all.stream().filter(l -> l.date != null)
                                                                             .map(l ->
            {
                ...
                {
                    commentsStream = commentsStream.filter(m -> m.author.isNull() == false && m.author.accountId
                            .equals(l._accountId));
                }
                ...
                if (comment.isPresent())
                {
                    ...
                    if (type != null)
                    {
                        Optional<ApprovalValueDto> value = type.values.stream().filter(v -> Integer.compare(v.value,
                                l.value) == 0).findFirst();
                        if (value.isPresent())
                        {
    ...
    private void addPatchSetComments(ChangeDto change, PatchSetDto patch, Map<String, List<CommentInfo>>
            revisionComments) throws SQLException
    {
        for (Map.Entry<String, List<CommentInfo>> fc : revisionComments.entrySet())
        {
            Optional<PatchSetFileDto> opt = patch.files.stream().filter(f -> f.path.equals(fc.getKey()))
                                                       .findFirst();
            if (opt.isPresent())
            {
            ...
            {
                logger.info(Logging.prepareWithPart("saveChange", "comments: file not found", Integer.toString
                        (change.id)));
            }
        }
    }

    private void addPatchSetMessages(ChangeDto change, List<ChangeMessageInfoEx> messages, PatchSetDto patch) throws
            SQLException
    {
        for (ChangeMessageInfoEx m : messages)
        {
            if (m != null && m._revisionNumber != null && m._revisionNumber.equals(patch.number))
            {
    ...
    private void addChangeMessages(ChangeDto change, List<ChangeMessageInfoEx> messages) throws SQLException
    {
        for (ChangeMessageInfoEx m : messages)
        {
            if (m != null && m._revisionNumber == null)
            {
