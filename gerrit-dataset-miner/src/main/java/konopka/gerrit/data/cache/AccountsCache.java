...
public class AccountsCache
{
    ...
    {
        if (isCachedByGerritId(gerritId))
            return cacheByGerritId.get(gerritId);
        if (isCachedByEmail(email))
            return cacheByEmail.get(email);
        if (isCachedByName(name))
    ...
    public void cache(AccountDto account)
    {
        ...
        } else
        {
            ...
            if ((account.name != null) && isCachedByName(account.name) == false)
                cacheByName.put(account.name, account);
        }
    }
