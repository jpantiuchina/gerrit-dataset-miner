...
public class GerritMiner
{
    ...
    private static final Logger logger = LoggerFactory.getLogger(GerritMiner.class);
    ...
    GerritMiner(Configuration config) throws Exception
    {
    ...
    private boolean canMineChange(ChangeDownloadDto download, int attempt, int maxAttempts)
    {
        if (attempt < maxAttempts)
        {
            ...
            {
                case DOWNLOADED:
                    logger.info(Logging.prepareWithPart("canMineChange", "skip downloaded", Integer.toString(download
                            .getChangeId())));
                    return false;
                ...
                case NOT_FOUND:
                    if (attempt > 1)
                    {
                        logger.info(Logging.prepareWithPart("canMineChange", "skip not found", Integer.toString
                                (download.getChangeId())));

                        return false;
                    }
                case ERROR:
                    boolean skip = download.getLastAttempt().toLocalDateTime().isBefore(LocalDateTime.now().minusDays
                            (3));
                    if (skip == false)
                    {
                        logger.info(Logging.prepareWithPart("canMineChange", "skip missing", Integer.toString
                                (download.getChangeId())));
                    }
    ...
    private DownloadResult mineChange(int id) throws SQLException
    {
    ...
    private void checkChange(ChangeDownloadDto download) throws SQLException
    {
        DownloadResult result = download.getResult();
    ...
    private boolean tryMineChange(int id, int maxAttempts) throws SQLException
    {
                ...
                if (count > 0)
                {
                    int savedChanges = infos.stream().
                            map(i -> {
                                try
                                {
                                    return tryMineChange(i._number, maxAttempts);
                                }
                    ...
                    logger.info(Logging.prepareWithPart("mineQuery", String.format("changes %d/%d (%f)",
                            savedChanges, count, (float) savedChanges / count)), start, limit);
    ...
    private void mineSimple(int start, int stop)
    {
        int id = start;
        int count = 0;
        int savedChanges = 0;
        ...
        try
        {
            ...
            for (; id < stop; id += 1)
            {
                ...
                if (count % 100 == 0)
                {
                    logger.info(Logging.prepareWithPart("mineSimple", String.format("changes %d - %d: %d/%d (%f)", id
                            - count, id, savedChanges, count, (float) savedChanges / count)), start, stop);
                    count = 0;
                    savedChanges = 0;
                }
            }
